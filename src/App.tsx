...
type ModelPrediction = { confidence: number; probabilities: Record<string, number>; heatmap?: string }
    ...
    <div className="min-h-screen bg-[#f7faf9] text-slate-900">
      ...
      <main className="lg:pl-[252px]">
        ...
        <div className="mx-auto max-w-[1500px] px-5 py-7 lg:px-9 lg:py-9">
          ...
          {page === 'report' && <ReportPage patient={openedPatientDetail} detail={detail} online={online} onRefer={() => { setPage('workflow'); setStep(4); setShowReferral(true) }} />}
...
function ReportPage({ patient, detail, online, onRefer }: { patient: Patient; detail: GradeDetails; online: boolean; onRefer: () => void }) {
  const [printed, setPrinted] = useState(false)
  return <><div className="flex flex-col justify-between gap-4 md:flex-row md:items-end"><div><p className="eyebrow">FHIR / ABDM-ready prototype</p><h1 className="page-title">Screening report</h1><p className="mt-2 text-sm text-slate-600">Structured as an interoperable screening record — no live ABDM, ABHA, or FHIR server integration.</p></div><div className="flex gap-3"><button onClick={() => { window.print(); setPrinted(true) }} className="btn-secondary">Print / Save PDF</button><button onClick={onRefer} className="btn-primary">Refer to specialist</button></div></div>{printed && <div className="mt-5 rounded-xl border border-teal-200 bg-teal-50 px-4 py-3 text-sm font-semibold text-teal-900">Print dialog requested. In a production integration, this would export a signed FHIR-compatible report.</div>}<div className="mt-6"><ReportViewer patient={patient} detail={detail} online={online} /></div><div className="mt-6 grid gap-4 md:grid-cols-3"><InfoPanel title="Patient" icon="P" text={`${patient.id} · screening date ${patient.screenDate}`} /><InfoPanel title="Observation" icon="O" text={`${detail.label} evidence and recommendation represented as prototype record fields.`} /><InfoPanel title="Service request" icon="R" text="Referral status can be carried to a specialist workflow when integrated." /></div></>
}
