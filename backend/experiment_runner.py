...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--exp-id", type=str, required=True, help="Unique experiment ID (e.g., E01_randaug)")
...
def main() -> None:
    ...
    if args.sampler == "none":
        train_loader = DataLoader(
            FundusDataset(training_samples, train_transform),
    ...
    validation_loader = DataLoader(FundusDataset(validation_samples, eval_transform), batch_size=args.batch_size, shuffle=False, num_workers=args.workers, pin_memory=pin_memory)
    ...
    if args.sanity_check:
        ...
        # Run 2 training steps to verify CUDA forward/backward/loss
        model.train()
        for batch_idx, (images, labels) in enumerate(train_loader):
            ...
            scaler.update()
            print(f"[Sanity Step {batch_idx+1}/2] Loss: {loss.item():.4f} (Finite: {torch.isfinite(loss).item()})")
    ...
    test_loader = DataLoader(FundusDataset(test_samples, eval_transform), batch_size=args.batch_size, shuffle=False, num_workers=args.workers, pin_memory=pin_memory)
